...
async function awsFargateCommand(ins, outs, config, cb) {
    ...
    while (runningTasks >= 500) { // AWS Fargate supports up to 500 instances at a given time
        await sleep(3000);
    }
    while ((Date.now() - times[index]) < 10000) {
    ...
    const functionType = config.deploymentType ? config.deploymentType : executor_config.functionType;
...
//actuall Container Overrides length must be at most 8192 so changed value to compare to 8000
    if (jobMessage.length > 7000) { // if payload is bigger than 8192 bytes (npm argument size limit), it is send via S3
    ...
    async function runTask() {
        ...
        await ecs.runTask(await createFargateTask()).promise().then(async function (data) {
            ...
            if (containerStatusCode !== 0) {
                console.log("Error: container returned non-zero exit code: " + containerStatusCode + " for task " + config.id + " with arn: " + taskArn + " with data: " + JSON.stringify(data));
                return
            }
            const params = {
                Bucket: options.bucket,
            ...
            console.log("Fargate task: " + config.id + " with arn: " + taskArn + " completed successfully.");
            //if (executor_config.metrics) {
            const log = await s3.getObject(params).promise().then(data => data.Body.toString());
            console.log("FargateFunction: " + config.name + " id: " + config.id + " resource: " + functionType + " fired: " + fireTime + " data: " + log);
            //}
            runningTasks--;
            cb(null, outs);
        });
    ...
    async function createFargateTask() {
        ...
        return {
            ...
            cluster: executor_config.cluster_arn[functionType],
            count: 1,
            enableECSManagedTags: false,
            launchType: "FARGATE",
            networkConfiguration: {
                awsvpcConfiguration: {
                    subnets: [executor_config.subnet_1[functionType], executor_config.subnet_2[functionType]],
    ...
    async function getTaskDefinition() {
        ...
        let taskDefinition = mapping[functionType];
