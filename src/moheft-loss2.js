    ...
    decorateStrategy(dag, cache = true) {
        ...
        for (const timeSolution of timeSolutions) {
            ...
            for (let i = 0; i < weights.length; i++) {
                ...
                let timeOfSchedule = this.getExecutionTimeOfSchedule(timeSolution);
                if (costOfSchedule <= userBudget && timeOfSchedule <= userDeadline) {
                    solutionsWithTimeAndCost.push([timeOfSchedule, costOfSchedule, timeSolution]);
                    if(i > 0){
                ...
                let newTime= this.getExecutionTimeOfSchedule(timeSolution);
                if (newCost <= userBudget && newTime <= userDeadline) {
                    console.log("Found, improved MOHEFT!" + this.config.budgetParameter + ";" + this.config.deadlineParameter);
                    // console.log(newCost);
                    // console.log(this.getExecutionTimeOfSchedule(timeSolution));
