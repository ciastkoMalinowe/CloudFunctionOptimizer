...
module.exports.executor = function (event, context) {
  event.Records.forEach(record => {
    console.log(record);
    const body = JSON.parse(record.body).body;
    console.log(body);
    ...
    function uploadResult(callback) {
      ...
      console.log("Uploading " + bucket_name + "/log/" + record.messageId);
      
      const params = {
          Bucket: body.options.bucket,
          Key: "logs/" + record.messageId,
          Body: response
        };
      console.log("Trying to save logs");
      s3.putObject(params, function (err) {
        if (err) {
          console.log("Error uploading file " + record.messageId);
          console.log(err);
          callback(err);
          return;
        }
        console.log("Uploaded file " + record.messageId);
        callback();
      });
    }
